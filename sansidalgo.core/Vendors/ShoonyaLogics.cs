...
namespace sansidalgo.core.Vendors
{
    public class ShoonyaLogics : IShoonyaLogics
    {
        ...
        private static readonly string[] Summaries = new[]
      {
        ...
        public async Task<string> ExecuteShoonyaOrder(TblOrderSetting order,decimal IndexPrice)
        {
            ...
            try
            {
                ...
                if (!Summaries.Any(w => w.Equals(order.BrokerCredentials.Uid)))
                {
                    return await Task.FromResult(string.Concat("UID: ", order.BrokerCredentials.Uid, " is not registered"));
                }
                nApi = new NorenRestApi();
                var endPoint = "https://api.shoonya.com/NorenWClientTP/";
                LoginMessage loginMessage = new LoginMessage();
                loginMessage.apkversion = "1.0.0";
                loginMessage.uid = order.BrokerCredentials.Uid;
                loginMessage.pwd = order.BrokerCredentials.Password;
                loginMessage.vc = order.BrokerCredentials.Vc;
                loginMessage.appkey = order.BrokerCredentials.ApiKey;
                loginMessage.imei = order.BrokerCredentials.Imei;
                loginMessage.source = "API";
                logger.Info("getting OTP Started");
                int cntr = 0;
            retryOtp:
                OtpEntity oe = await helper.GetTOTP(order.BrokerCredentials.AuthSecreteKey);
                while (oe.RemaingTime < 2)
                {
                    oe = await helper.GetTOTP(order.BrokerCredentials.AuthSecreteKey);
                }
                loginMessage.factor2 = oe.OTP;
                ...
                TimeSpan startTime = TimeSpan.Parse(order.OptionsSettings.StartTime);
                TimeSpan endTime = TimeSpan.Parse(order.OptionsSettings.EndTime);
                // Get the current time
                // Convert the local time to IST
                DateTime currentTime = TimeZoneInfo.ConvertTime(DateTime.Now, istTimeZone);
                ...
                var openOrders = nApi.SendGetPositionBook(responseHandler.OnResponse, order.BrokerCredentials.Uid);
                await Task.FromResult(responseHandler.ResponseEvent.WaitOne());
                if (responseHandler.baseResponse != null)
                {
                    var bookResponse = responseHandler.baseResponse as PositionBookResponse;
                    ...
                    if (bookResponse != null && bookResponse.stat != "Not_Ok")
                    {
                        ...
                        foreach (var p in openPositions)
                        {
                            ...
                            try
                            {
                                ...
                                placeOrder = new PlaceOrder();
                                placeOrder.uid = order.BrokerCredentials.Uid;
                                placeOrder.actid = order.BrokerCredentials.Uid;
                                placeOrder.exch = order.OptionsSettings.Exchange;
                                placeOrder.tsym = p.tsym;
                                ...
                                if (p.tsym.ToUpper().StartsWith(order.OptionsSettings.Instrument.Name.ToUpper()) && p.tsym.Substring(p.tsym.Length - 8).Contains("P") && (order.OrderSide.Name == "pesell" || order.OrderSide.Name == "cebuy"))
                                {

                                    placeOrder.trantype = "S";

                                }
                                else if (p.tsym.ToUpper().StartsWith(order.OptionsSettings.Instrument.Name.ToUpper()) && p.tsym.Substring(p.tsym.Length - 8).Contains("C") && (order.OrderSide.Name == "cesell" || order.OrderSide.Name == "pebuy"))
                                {
                ...
                if (placeNewOrder && order.OrderSide.Name.Contains("buy"))
                {
                    ...
                    placeOrder.actid = order.BrokerCredentials.Uid;
                    placeOrder.exch = order.OptionsSettings.Exchange;
                    placeOrder.tsym = await helper.GetFOAsset(order.OptionsSettings.Instrument.Name, Convert.ToInt32(order.OptionsSettings.CeSideEntryAt), IndexPrice, order.OrderSide.Name, order.OptionsSettings.ExpiryDay);
                    logger.Info("Prepared Asset: " + Convert.ToString(placeOrder.tsym));
                    placeOrder.qty = Convert.ToString(order.OptionsSettings.PlayQuantity);
                    placeOrder.dscqty = "0";
                    placeOrder.prd = "M";
